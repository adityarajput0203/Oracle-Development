CREATE OR REPLACE PACKAGE BODY APPS.ADOR_AP_INV_IMPORT_DTL_PKG
...
IS
   ...
   /*************************************************************************/
   PROCEDURE ADOR_INV_VERIFY_P (RETCODE OUT VARCHAR2, ERRBUF OUT VARCHAR2)
   IS
      ...
      IS
         SELECT INVOICE_NUM,
                ...
                LIABILITY_ACCOUNT,
                ORGANIZATION_CODE
           FROM CUS.ADOR_AP_INV_HDR_STG_T
      ...
      IS
         SELECT INVOICE_NUM,
                ...
                SAC_CODE,
                HSN_CODE
           FROM CUS.ADOR_AP_INV_LINE_STG_T
          WHERE REC_STATUS IS NULL;
      ...
      BEGIN
         ...
         THEN
            ...
            LOOP
               RETCODE := '0';
               L_ERR_MSG :=
                     'Message from ADOR_AP_INVOICE_PKG.ADOR_INVOICE_VERIFY:'
               ...
               /*
                  SELECT V.ORGANIZATION_ID
                   ...
                   WHERE     M.COST_ORGANIZATION_ID = V.ORGANIZATION_ID
					...
					WHERE 1 = 1                      
						AND (UPPER (V.NAME) = UPPER ( T_CUR_APINV_MAIN_REC (I).ORGANIZATION_CODE));
                  IF L_ORG_ID IS NULL
                  THEN
                     L_ERR_MSG :=
                           RTRIM (L_ERR_MSG)
				...
				/*---------------------------Validation of invoice number -------------------------------------*/
                  BEGIN
                     SELECT INVOICE_NUM
                      ...
                      WHERE     1=1
                        AND INVOICE_NUM = T_CUR_APINV_MAIN_REC (I).INVOICE_NUM
                        AND ORG_ID = L_ORG_ID;
                     ...
                     THEN
                        L_ERR_MSG :=
                           RTRIM (L_ERR_MSG) || ' ,I01- INVOICE ALREADY EXISTS';
                        RETCODE := '2';
                     END IF;
                  EXCEPTION
                     WHEN OTHERS
               ...
               BEGIN
                  SELECT CODE
                   ...
                   WHERE CODE = T_CUR_APINV_MAIN_REC (I).DOC_CATEGORY_CODE;
               EXCEPTION
                  WHEN OTHERS
                  THEN
                     L_ERR_MSG :=
               ...
               BEGIN
                  SELECT SOURCE
                   ...
                   WHERE     INVOICE_NUM =
                                T_CUR_APINV_MAIN_REC (I).INVOICE_NUM
                         AND SOURCE IN ('AP-MIGRATION');		--Need to change at the time of running the request on PROD
               ...
               BEGIN
                  SELECT COUNT (TAX_CATEGORY)
                    INTO TX_CNT
                    FROM CUS.ADOR_AP_INV_LINE_STG_T T
                   WHERE     1 = 1
                         AND T.INVOICE_NUM =
                                T_CUR_APINV_MAIN_REC (I).INVOICE_NUM;
               ...
               THEN
                  ...
                  UPDATE CUS.ADOR_AP_INV_HDR_STG_T T
      ...
      BEGIN
         ...
         THEN
            ...
            LOOP
               RETCODE := '0';
               L_ERR_MSG :=
                     'Message from ADOR_AP_INVOICE_DTL_PKG.ADOR_INVOICE_VERIFY:'
               ...
               EXCEPTION
                  ...
                  THEN
                     L_ERR_MSG :=
                           RTRIM (L_ERR_MSG)
                        || ' ,I01- INVOICE DOES NOT EXIST INSI-FY25-00943-B2 HEADER ';
                     RETCODE := '2';
               END;
               ...
               THEN
                  ...
                  --    apps.fnd_file.put_line(apps.fnd_file.output,ERRBUF ||'and'||L_ERR_MSG);
                  UPDATE CUS.ADOR_AP_INV_LINE_STG_T T
                     SET T.REC_STATUS = 'E', T.ERR_MESSAGE = L_ERR_MSG
                   WHERE     t.INVOICE_NUM =
                                T_CUR_APINV_MAIN_LINE_REC (I).INVOICE_NUM
                         AND t.LINE_NUMBER =
   ...
   END ADOR_INV_VERIFY_P;


   PROCEDURE ADOR_INV_MAIN_INSERT_P (RETCODE   OUT VARCHAR2,
                                     ERRBUF    OUT VARCHAR2)
   IS
      CURSOR CUR_APINV_MAIN
      IS
   ...
   END ADOR_INV_MAIN_INSERT_P;


END ADOR_AP_INV_IMPORT_DTL_PKG;
/
