CREATE OR REPLACE PACKAGE BODY APPS.xxccfs_gg_filter_condition_pkg
/******************************************************************************
   NAME:       xxccfs_gg_filter_condition_pkg.pkb
   ...
   1.2        11-06-2025   XW294               Removed old new values and restructured
                                               code
  1.3        26-08-2025  XW294                Changed get_cp_Count Function and added
                                             client_id condition in prc. (CHG0250437)
   ASSUMPTIONS:
   LIMITATIONS:
   ALGORITHM:
   NOTES:
...
'
      ...
      THEN
         ...
         -- Changed the is_ind_org function as per requirment.by Aditya R on 12-Jun-2025 CRQ CHG0242317
      ...
      THEN
           ...
           --l_eligible_count := 1;
           --Added by Aditya R to check wherther grant_type,grant_Id is india specific on 13-Jun-2025 for CRQ CHG0242317
    ...
    and hou.set_of_books_id = 2042
                 --  and HOU.name in ('CTT India Dewas Operating Unit','CTT India Pithampur Operating Unit')
                     )); */
      -- commented by Aditya R on 10-Jun-25 for CRQ CHG0242317
      ...
      -- Commented By Aditya R on 10-Jun-25 CRQ CHG0242317
          ...
          WHERE EXISTS
                   (SELECT 1
                     ...
                     WHERE     APS.VENDOR_ID = TO_NUMBER (p_key_value7) -- VENDOR_ID
                           ...
                           AND hou.set_of_books_id = 2042 --AND  HOU.name in ('CTT India Dewas Operating Unit','CTT India Pithampur Operating Unit')
                                                         );
      ...
      -- Commented by Aditya R on 11-Jun-25 for CRQ CHG0242317
      /*
  ...
  and hou.set_of_books_id = 2042
   ...
   and hou.set_of_books_id = 2042
      ...
      THEN
         ...
         -- p_key_value8 = ORGANIZATION_ID
         l_eligible_count := is_ind_org (NULL, p_key_value8);
      ELSIF p_table_name = 'PO_SYSTEM_PARAMETERS_ALL' --Added by Aditya R on 12-Jun-25 CRQ CHG0242317
      THEN
         -- p_key_value7 = org_id
         l_eligible_count := is_ind_org (p_key_value7, NULL);
      ELSIF (p_table_name = 'GL_PERIOD_TYPES') --Added by Aditya R on 12-Jun-25 CRQ CHG0242317
      THEN
      ...
      ELSIF (p_table_name = 'GL_PERIODS') --Added by Aditya R on 12-Jun-25 CRQ CHG0242317
      THEN
         ...
         THEN
            l_eligible_count := 1;
         END IF;
      ELSIF P_TABLE_NAME IN ('RA_ACCOUNT_DEFAULT_SEGMENTS', --Added by Aditya R on 12-Jun-25 CRQ CHG0242317
                             'FND_FLEX_VALUES_TL',
                             'FND_FLEX_VALUES',
                             'AP_BANK_BRANCHES')
      THEN
   ...
   FUNCTION get_cp_count (p_prog_val IN VARCHAR2, p_client_id IN VARCHAR2)    -- Changed by Aditya R on 26-Aug-2025 for CHG0250437
      RETURN NUMBER
   IS
      ln_cp_count   NUMBER;
   BEGIN
      SELECT COUNT (1)
        INTO ln_cp_count
        FROM DUAL
       WHERE (p_client_id = 'INTERFACE' and p_prog_val like 'sqlplus%')         -- Added by Aditya R on 22-Aug-2025 for not capturing Interface Records for CHG0250437
       OR    EXISTS
                (SELECT 1
                  ...
                  WHERE (   program = fcp.concurrent_program_name
                         ...
                         OR p_prog_val LIKE 'rwrun%';                           --- Added by Aditya R 21-Aug-2025 for CHG0250437
   ...
   /******************************************************************************
      NAME:        xxccfs_gg_filter_condition_prc
      PURPOSE:
   ...
   1.1  17-04-2025  AL02R     Added the table and make india vaidation on SET_OF_BOOKS_ID
      ...
      NOTES:
   ******************************************************************************/
   PROCEDURE xxccfs_gg_filter_condition_prc (
   ...
   AS
      ...
      PROCEDURE xxccfs_insert_debug_log (p_log_object       VARCHAR2,
      ...
      BEGIN
         l_parameter_mess :=            -- Added by Aditya R on 9-Jun-2025 CRQ CHG0242317
            (   ' Event is '
             || p_event
             || ' , Client ID is '
             || p_client_id
   ...
   BEGIN
      ...
      ln_count := get_cp_count (p_prog_val,p_client_id); -- Added by Aditya R on 11-Jun-2025 for CRQ CHG0242317

      IF ln_count = 0
      THEN
         IF p_event IN ('DELETE', 'INSERT')
         THEN             -- If the record is deleted or inserted from backend
            --Added by Aditya R on 22-Aug-2025 for CHG0250437
         ...
         ELSE
            ...
            THEN
               ...
               ELSE
                  ...
                  --ln_count := 0;          --No need to set.     -- Changed by Aditya R on 20-Aug-2025 for CHG0250437
            ...
            --            --  ELSIF  p_table_name = 'GL_BALANCES' AND p_client_id IS NULL     -- Commented by Aditya R 10-Jun-25 for CRQ CHG0242317
            --            --  THEN
            --            --    ln_count :=1;
            --added by Aditya R on 20-Aug-2025 added table in above condition
            ELSIF p_table_name = 'WSH_DELIVERY_DETAILS'
            THEN
               IF p_client_id IS NULL AND (p_prog_val NOT LIKE 'frmweb%' AND p_prog_val != 'JDBC Thin Client')  -- Changed by Aditya R for CHG0250437
               THEN
                  ...
                  IF ln_count = 1
                  THEN
                     l_status_message := 'Record is updated from backend';
                  ELSE
                     l_status_message :=
                        'Record is updated from backend but not India specific';
                  END IF;
...
'
         ...
         and hou.set_of_books_id = 2042
            ...
            -- Commented by Aditya R 10-Jun-25 for CRQ CHG0242317
      ...
      xxccfs_insert_debug_log (p_table_name,
                               lc_key_value,
                               ln_count,
                               -- lc_old_val,
                               -- lc_new_val,
   ...
   EXCEPTION
      ...
      THEN
         ...
         xxccfs_insert_debug_log (p_table_name,
                                  ...
                                  -- lc_new_val,
                                  l_status_message);
   END xxccfs_gg_filter_condition_prc;
END xxccfs_gg_filter_condition_pkg;
/
